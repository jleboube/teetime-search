...
def render(payload: dict) -> str:
    ...
    for key, label in BAND_LABELS.items():
        ...
        for group in groups:
            ...
            price = (
                f"${best['price_per_player']:.0f}"
                if best["price_per_player"] is not None
                else "  —"
            )
            note = ""
            ...
            others = {
                ...
                for x in listings
                if x["provider"] != best["provider"]
                and x["price_per_player"] is not None
            }
            if others:
                alt = ", ".join(f"${v:.0f} on {k}" for k, v in others.items())
                note = f"  (also {alt})"
            if best.get("price_confidence") == "low":
                note += "  [price approximate]"
